...
import Text.ParserCombinators.Parsec (Parser, parse, try, (<|>), char, digit, string, many1, sepEndBy1, sepEndBy)
...
signalsParser :: Parser [(Signal, Signal)]
signalsParser = sepEndBy1 ((,) <$> signalParser <*> (char '\n' *> signalParser)) (try $ string "\n\n")
...
main = do
  ...
  print $ sum $ map fst $ filter (validate . snd) $ zip [1..] signals
