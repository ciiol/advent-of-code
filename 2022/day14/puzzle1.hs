...
lowerLimit = (+ 1) . maximum . map snd . S.toList
...
simulate maxY s (c:cs) = s : maybe [] (const $ simulate maxY s' cs) (wrap end)
  where
    end = last $ takeWhile (isNotLower (maxY + 1)) $ traceSand s c
    wrap x | isNotLower maxY x = Just x
    wrap x = Nothing
    s' = S.insert end s

simulationLen :: [Line] -> Int
simulationLen l = len - 1
  where
    occupied = makeOccupied l
    maxY = lowerLimit occupied
    len = length $ simulate maxY occupied (repeat (500, 0))
