...
-- largest number of geodes that can be opened in 24 minutes using that blueprint.
...
possibleRobots state = mapMaybe steps robots
  where
    ...
    isNotTooMany r = (10 >) $ fromMaybe 0 $ M.lookup r $ factory state
    isNotOverProduced = (25 >) . fromMaybe 0 . (`M.lookup` resources state)
...
quality t b@(Blueprint i _) = i * simulate (makeState b 1) t
...
main = do
  ...
  print $ sum $ map (quality 24) blueprints
