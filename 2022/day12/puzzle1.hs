...
findStart = fst . head . filter ((== Start) . snd) . M.toList

findPath :: Coord -> (Elem -> Bool) -> Heightmap -> [Coord]
findPath start isEnd h = bfs (M.fromList [(start, start)]) (M.fromList [((0, start), (0, start))])
  where
    ...
    bfs v m | isEnd (at h candidate) = reverse $ unwind v candidate
      where
...
main = do
  ...
  print $ (+ (-1)) . length $ findPath (findStart heightmap) (== End) heightmap
