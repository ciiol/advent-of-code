...
renderPage page = intercalate "\n" lines
  where
    maxY = maximum $ map snd $ S.toList page
    maxX = maximum $ map fst $ S.toList page
    minY = minimum $ map snd $ S.toList page
    minX = minimum $ map fst $ S.toList page
    lines = [[render (x, y) | x <- [minX..maxX]] | y <- [minY..maxY]]
    render coord | coord `S.member` page = '#'
    render _coord = '.'
...
main = do
  ...
  putStrLn $ renderPage $ foldl' foldPage page foldings
