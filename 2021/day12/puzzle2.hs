...
data Node = Start | End | Small String | Large String deriving (Eq, Ord, Show)
...
pathes g start end = pathes' g [start] end
  where
    ...
    pathes' g path end | not $ isValid False path = []
    pathes' g path end = concatMap (\x -> pathes' g (x : path) end) (next path)
    isValid _ [] = True
    isValid False (n@(Small _) : path) | n `elem` path = isValid True path
    isValid True (n@(Small _) : path) | n `elem` path = False
    isValid onlyOnce ((Large _) : path) = isValid onlyOnce path
    isValid _ (n@Start : path) | n `elem` path = False
    isValid _ (n@End : path) | n `elem` path = False
    isValid onlyOnce (_ : path) = isValid onlyOnce path
...
main = do
  ...
  print $ length $ pathes graph Start End
