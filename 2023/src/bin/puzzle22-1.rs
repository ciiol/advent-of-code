...
impl Space {
    ...
    fn is_supported_only_by(&self, brick: &Brick, other: &Brick) -> bool {
        if brick.a.z == 1 || brick.b.z == 1 {
            return false;
        }
        if !self.is_supported(brick) {
            return false;
        }
        brick.points_below().into_iter().all(|p| {
...
fn puzzle(input: &[Brick]) -> usize {
    ...
    space
        ...
        .iter()
        .filter(|b| {
            let supports = space.supports(b);
            supports
                .into_iter()
                .all(|s| !space.is_supported_only_by(s, b))
        })
        .count()
}
...
mod tests {
    ...
    fn test_puzzle() -> Result<(), Error> {
        let input = "1,0,1~1,2,1\n\
                     ...
                     0,1,6~2,1,6\n\
                     1,1,8~1,1,9";
        assert_eq!(puzzle(&parse(input, puzzle_input)?), 5);
        Ok(())
    }
}
