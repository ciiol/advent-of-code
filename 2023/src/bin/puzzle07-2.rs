...
// hand the strongest type possible. To balance this, J cards are now the weakest
// individual cards, weaker even than 2.
//
...
fn make_hand(cards: Vec<Card>) -> Hand {
    ...
    let jokers = card_count.get(&Card::J).copied().unwrap_or(0);
    let mut counts: Vec<usize> = card_count
        .into_iter()
    ...
    match counts.as_slice() {
        [] if jokers == 5 => Hand::FiveOfAKind(cards),
        [n] if n + jokers == 5 => Hand::FiveOfAKind(cards),
        [n1, n2] if n1 + n2 + jokers == 5 && n2 + jokers == 4 => Hand::FourOfAKind(cards),
        [n1, n2] if n1 + n2 + jokers == 5 && n2 + jokers == 3 => Hand::FullHouse(cards),
        [1, 1, n3] if 2 + n3 + jokers == 5 && n3 + jokers == 3 => Hand::ThreeOfAKind(cards),
        [1, n1, n2] if 1 + n1 + n2 + jokers == 5 => Hand::TwoPair(cards),
        [1, 1, 1, n4] if 3 + n4 + jokers == 5 => Hand::OnePair(cards),
...
mod tests {
    ...
    fn test_puzzle() -> Result<(), Error> {
        let input = "32T3K 765\n\
                     ...
                     KTJJT 220\n\
                     QQQJA 483";
        assert_eq!(puzzle(&parse(input, puzzle_input)?), 5905);
        Ok(())
    }
}
