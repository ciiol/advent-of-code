...
// the next values of each previous sequence, you should instead add a zero to the beginning
// of your sequence of zeroes, then fill in new first values for each previous sequence.
//
...
fn predict(history: &[Measurement]) -> Measurement {
    ...
    let first_values = histories.iter().map(|h| h.first().unwrap());
    first_values.rfold(0, |acc, &v| v - acc)
}
